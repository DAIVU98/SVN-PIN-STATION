...
class YAMLHighlighter(QSyntaxHighlighter):
    def __init__(self, parent=None):
        ...
        keyFormat.setForeground(QColor("#0044aa"))
        keyFormat.setFontWeight(QFont.Bold)
        self.highlightingRules.append((QRegExp(r"^[ \t]*[a-zA-Z0-9_-]+(?=:)"), keyFormat))

        # String format (quoted or unquoted)
        stringFormat = QTextCharFormat()
        stringFormat.setForeground(QColor("#cc5500"))
        self.highlightingRules.append((QRegExp(r'"[^"]*"'), stringFormat))
        self.highlightingRules.append((QRegExp(r"'[^']*'"), stringFormat))

        # Number format
        numberFormat = QTextCharFormat()
        numberFormat.setForeground(QColor("#aa00aa"))
        self.highlightingRules.append((QRegExp(r"\b[+-]?[0-9]+(\.[0-9]*)?\b"), numberFormat))

        # Boolean format
        boolFormat = QTextCharFormat()
        boolFormat.setForeground(QColor("#008800"))
        self.highlightingRules.append((QRegExp(r"\b(true|false)\b", Qt.CaseInsensitive), boolFormat))

        # Null format
        nullFormat = QTextCharFormat()
        nullFormat.setForeground(QColor("#888888"))
        self.highlightingRules.append((QRegExp(r"\b(null|~)\b", Qt.CaseInsensitive), nullFormat))

        # Comment format
        commentFormat = QTextCharFormat()
        commentFormat.setForeground(QColor("#999999"))
